...
export const reportSchema = z.object({
  ...
  reporterName: z.string().trim().min(2, "Escribe tu nombre.").max(140), phone: z.string().trim().min(7, "Escribe un número de celular válido.").max(40).regex(/^[0-9+\s()-]+$/, "Escribe solo un número de celular válido."), captchaToken: z.string().max(2048).optional(), website: z.string().max(0).optional()
});

export const informationSchema = z.object({ reportType: z.enum(["sighting", "possible_trapped", "possible_deceased", "correction", "other_information"]), eventAt: z.string().optional(), location: z.string().max(240).optional(), description: z.string().trim().min(10, "Describe la información con un poco más de detalle.").max(3000), consent: z.literal(true), captchaToken: z.string().max(2048).optional(), website: z.string().max(0).optional() });
