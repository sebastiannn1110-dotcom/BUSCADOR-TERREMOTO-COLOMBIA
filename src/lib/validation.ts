...
export const reportSchema = z.object({
  ...
  reporterName: z.string().trim().min(2, "Escribe tu nombre.").max(140), phone: z.string().trim().min(7, "Escribe un número de celular válido.").max(40).regex(/^[0-9+\s()-]+$/, "Escribe solo un número de celular válido."), website: z.string().max(0).optional()
});
