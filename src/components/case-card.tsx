  ...
  return <article className="case-card">
    <Link href={`/persona/${item.slug}`} className="card-link" aria-label={`Ver caso de ${item.full_name}`}>
      ...
      <div className="card-body">
        {item.is_test_data && <p className="test-label">DATOS DE PRUEBA</p>}
    ...
    <div className="card-actions">
      <Link className="button secondary" href={`/persona/${item.slug}`}>Ver caso</Link>
      <Link className="button" href={`/persona/${item.slug}/informacion`}>Tengo información</Link>
    </div>
  </article>;
}
